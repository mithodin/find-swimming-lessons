...
export class SwmDownloader {
  ...
  private static readonly BOOKINGS_SEARCH = (tab: number) =>
    `/de/bookings/block_list_search_submit/bookable/0/tab/${tab}/`;
  ...
  public async searchCourses(params: CourseSearchParams) {
    ...
    data.set(
      ...
      `${params.search_number_of_periods ?? ""}`,
    );
    ...
    const url = new URL(
      SwmDownloader.BOOKINGS_SEARCH(params.active_tab_id),
      SwmDownloader.BASE_URL,
    );
